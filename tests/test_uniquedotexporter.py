...
def test_tree(tmp_path, root):
    """Tree."""
    UniqueDotExporter(root).to_dotfile(tmp_path / "tree.md")
...
def test_tree_filter(tmp_path, root):
    """Tree with Filter."""
    UniqueDotExporter(root, filter_=lambda node: node.name.startswith("sub")).to_dotfile(tmp_path / "tree_filter.md")
    assert_gen(tmp_path, REFDATA / "tree_filter")


def test_tree_stop(tmp_path, root):
    """Tree with stop."""
    UniqueDotExporter(root, stop=lambda node: node.name == "sub1").to_dotfile(tmp_path / "tree_stop.md")
    assert_gen(tmp_path, REFDATA / "tree_stop")


def test_tree_maxlevel(tmp_path, root):
    """Tree with maxlevel."""
    UniqueDotExporter(root, maxlevel=2).to_dotfile(tmp_path / "tree_maxlevel.md")
